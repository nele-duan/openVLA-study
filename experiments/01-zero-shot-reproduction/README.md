# Experiment 01 — Zero-Shot Reproduction (Pre-Finetuning)

Reproducing the **out-of-the-box** [OpenVLA-7B](https://github.com/openvla/openvla) checkpoint with **no finetuning**: loading the released weights, running single-step action prediction, and driving a closed-loop rollout inside the [LIBERO](https://github.com/Lifelong-Robot-Learning/LIBERO) simulation environment.

> Goal: establish a working baseline and confirm the full inference + simulation pipeline runs end to end before any task-specific finetuning.
...
- The released checkpoint runs zero-shot in LIBERO without any finetuning, but `libero_spatial` is **out of distribution** relative to its BridgeData training mix — useful as a baseline reference for the next experiment.
- This sets up the natural follow-up: **finetune on LIBERO** and compare rollout success against this baseline.
